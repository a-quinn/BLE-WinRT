...
//
// 20/10/2025 - Alastair Quinn
// This was pulled from https://github.com/adabru/BleWinrtDll
// It was one of the few simple working examples of a C++ DLL wrapping WinRT BLE functionality.
// It had a WTFPL license.
...
#define __WFILE__ L"BLE_WinRT.cpp"
...
{
	Characteristic_NewValue(characteristic, args.CharacteristicValue());
}
	...
	try {
		...
		if (characteristic != nullptr) {
			...
			if (status != GattCommunicationStatus::Success)
				...
				//auto v = co_await characteristic.ReadValueAsync();
				//v.Value();
				if (result != 0)
					*result = true;
			}
	...
	try {
		...
		if (characteristic != nullptr) {
			...
			if (status.Status() != GattCommunicationStatus::Success) {
				...
				Characteristic_NewValue(characteristic, status.Value());
				if (result != 0)
					*result = true;
			}
		}
...
void ClearCache(bool onlyServices) {	
