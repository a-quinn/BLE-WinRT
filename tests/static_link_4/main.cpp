...
/*
This test demonstrates static linking against the BLE_WinRT_Context.dll, using the BluetoothClient class.
The BluetoothClient class wraps the BLE_WinRT_Context functionality for easier use, something many users would do themselves.
*/
...
int main()
{
	...
	bool ret = client.scanForCharacteristic(serviceUuid, charUuid);
	if (!ret) {
	...
	while (startTime + duration > std::chrono::steady_clock::now()) {
		if (client.bl_device_->PollData(&data, false)) {
			std::wcout << L"Data received from " << data.deviceId << L" Service: " << data.serviceUuid << L" Char: " << data.characteristicUuid << L" Size: " << data.size << std::endl;
			std::wcout << L"  Bytes: ";
			for (int i = 0; i < data.size; ++i) std::wcout << std::hex << (int)data.buf[i] << L" ";
			std::wcout << std::endl;
